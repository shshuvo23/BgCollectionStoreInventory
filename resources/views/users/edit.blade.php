            ...
            <form action="{{ route('update_user',$user->id) }}" method="post">
                ...
                <div class="row">
                    <div class="col-lg">
                        ...
                        <input class="form-control" placeholder="User Name" value="{{ $user->name }}" type="text" name="name">
                        @error('name')
                    ...
                    <div class="col-lg mg-t-10 mg-lg-t-0">
                        ...
                        <input class="form-control" value="{{ $user->email }}" placeholder="Email" type="email" name="email">
                        @error('email')
                        <span class="text-danger">{{ $message }}</span>
                       @enderror
                    </div><!-- col -->
                ...
                <div class="row mg-t-20">
                    <div class="col-lg">
                        ...
                        <select class="form-control" name="role_id">
                            ...
                            <option value="{{ $role->id }}" {{ $role->role_id == $user->role_id?"selected":"" }}>{{ $role->role_name }}</option>
                            @endforeach
                            @error('role_id')
                            <span class="text-danger">{{ $message }}</span>
                          @enderror
